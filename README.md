...
| AGGREGATE_FILTERS                                                    | No         | N/A           | The resulted emails and testcases for each filter will be aggregated to a separate worksheet with name <WS>_aggregated_<aggregate-filter> where WS is equal to the value specified by the --gsheet-worksheet argument. |                                                                                                                                                                                                                                                                                                                                            |
| SKIP_AGGREGATION_RESOURCE_FILE                                       | No         | N/A           | Specify file that defines lines to skip. If lines starting with these strings, they will not be considered as a line to parse from the emails.                                                                         |                                                                                                                                                                                                                                                                                                                                            |
...
| GSHEET_COMPARE_WITH_JIRA_TABLE                                       | No         | N/A           | A value should be provided if comparison of failed testcases with reported Jira table must be performed. The value is a name to a Google Sheets worksheet, for example 'testcases with jiras'.                         |                                                                                                                                                                                                                                                                                                                                            |
...
### Environment variables for jobs: Google Sheet integration

Corresponding class: [BranchComparatorEnvVar](https://github.com/szilard-nemeth/yarn-dev-tools/blob/b484daffde3c6f70dc3dab71f92150738855d668/yarndevtools/cdsw/constants.py#L35-L38)

| Name                       | Level        | Mandatory? | Default value                      | Description                           |
|----------------------------|--------------|------------|:-----------------------------------|---------------------------------------|
| GSHEET_CLIENT_SECRET | Only for job | No         | origin/CDH-7.1-maint               | Name of the feature branch            |
| BRANCH_COMP_MASTER_BRANCH  | Only for job | Yes        | origin/cdpd-master                 | Name of the master branch             |
| BRANCH_COMP_REPO_TYPE      | Only for job | Yes        | downstream (`RepoType.DOWNSTREAM`) | Repository type. Can take a value of `RepoType` |
