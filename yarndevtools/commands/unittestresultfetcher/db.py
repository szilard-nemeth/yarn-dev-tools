...
class MailSendDataSchema(Schema):
    sent = fields.Boolean()
    sent_date = fields.DateTime(allow_none=True)

    @post_load
    def make_obj(self, data, **kwargs):
        return MailSendData(**data)


class MailSendDataForJobSchema(Schema):
    build_url = fields.Str(required=True)
    job_name = fields.Str(required=True)
    recipients = fields.Dict(keys=fields.Str, values=fields.Nested(MailSendDataSchema()))

    @post_load
    def make_obj(self, data, **kwargs):
        return MailSendDataForJob(**data)
...
@dataclass
class MailSendDataForJob(DBSerializable):
    build_url: str
    job_name: str
    recipients: Dict[str, MailSendData]

    def __post_init__(self):
        self._schema = MailSendDataForJobSchema()
    ...
    def add_recipient(self, recipient, sent, sent_date):
        if recipient not in self.recipients or not self.recipients[recipient].sent:
            self.recipients[recipient] = MailSendData(sent, sent_date)

    def reset(self):
        for mail_send_data in self.recipients.values():
            mail_send_data.reset()


class MailSentTracker(UserDict):
    def __init__(self, objs: List[MailSendDataForJob]):
        super().__init__()
        self.data: Dict[str, MailSendDataForJob] = {}  # Key: Job URL
        for obj in objs:
    ...
    def __setitem__(self, job_url, mail_send_data):
        self.data[job_url] = mail_send_data
    ...
    def get_val(self, job_url, job_name):
        ...
        if job_url not in self.data:
            self.data[job_url] = MailSendDataForJob(job_url, job_name, {})
        return self.data[job_url]

    def add_build_with_mail_data(self, job_url, job_name, recipient, sent, sent_date):
        if job_url not in self.data:
            self.data[job_url] = MailSendDataForJob(job_url, job_name, {recipient: MailSendData(sent, sent_date)})
        else:
    ...
    def reset_for_job(self, job_name):
        for mail_send_data_for_job in self.data.values():
            if mail_send_data_for_job.job_name == job_name:
                mail_send_data_for_job.reset()

    def mark_sent(self, job_url, job_name, recipients):
        dt = DateUtils.now()
        if job_url not in self.data:
            self.data[job_url] = MailSendDataForJob(job_url, job_name, {r: MailSendData(True, dt) for r in recipients})
        else:
            for r in recipients:
...
class UTResultFetcherDatabase(Database):
    MONGO_COLLECTION_EMAIL_SENT_DATA = "email_sent_data"

    def __init__(self, conf: MongoDbConfig):
        super().__init__(conf)
        self._jenkins_build_data_db = JenkinsBuildDatabase(conf)
        self._mail_send_data_for_job_schema = MailSendDataForJobSchema()
    ...
    def load_email_sent_state(self) -> List[MailSendDataForJob]:
        docs = self.find_all(collection_name=UTResultFetcherDatabase.MONGO_COLLECTION_EMAIL_SENT_DATA)
        objs: List[MailSendDataForJob] = []
        for doc in docs:
            obj: MailSendDataForJob = self._mail_send_data_for_job_schema.load(doc, unknown=EXCLUDE)
            objs.append(obj)
        return objs

    def save_email_sent_state(self, obj: MailSendDataForJob):
        if not obj:
            return
        self.save(
            obj,
            replace=True,
            collection_name=UTResultFetcherDatabase.MONGO_COLLECTION_EMAIL_SENT_DATA,
            id_field_name="build_url",
        )

    def reset_email_sent_state(self, tracker: MailSentTracker, job_names: List[str]):
        LOG.info("Resetting email sent state on the following jobs: %s", job_names)
        ...
        # TODO Specify id ?
        self.save_many(
            docs, collection_name=UTResultFetcherDatabase.MONGO_COLLECTION_EMAIL_SENT_DATA, id_field_name="build_url"
        )
