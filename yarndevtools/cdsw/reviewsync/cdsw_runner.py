...
class CdswRunner(CdswRunnerBase):
    def start(self, setup_result: CdswSetupResult, cdsw_runner_script_path: str):
        ...
        self.run_reviewsync_and_send_mail()

    def run_reviewsync_and_send_mail(self):
        self._run_reviewsync(
            gsheet_client_secret=OsUtils.get_env_value(ReviewSyncEnvVar.GSHEET_CLIENT_SECRET.value),
            gsheet_spreadsheet=OsUtils.get_env_value(ReviewSyncEnvVar.GSHEET_SPREADSHEET.value),
            gsheet_worksheet=OsUtils.get_env_value(ReviewSyncEnvVar.GSHEET_WORKSHEET.value),
            gsheet_jira_column=OsUtils.get_env_value(ReviewSyncEnvVar.GSHEET_JIRA_COLUMN.value),
            gsheet_update_date_column=OsUtils.get_env_value(ReviewSyncEnvVar.GSHEET_UPDATE_DATE_COLUMN.value),
            gsheet_status_info_column=OsUtils.get_env_value(ReviewSyncEnvVar.GSHEET_STATUS_INFO_COLUMN.value),
            branches=OsUtils.get_env_value(ReviewSyncEnvVar.BRANCHES.value),
        ...
        subject = f"YARN reviewsync report [start date: {self.start_date_str}]"
...
if __name__ == "__main__":
    ...
    mandatory_env_vars = [CdswEnvVar.MAIL_ACC_USER.value, CdswEnvVar.MAIL_ACC_PASSWORD.value] + [
        e.value for e in ReviewSyncEnvVar
    ]
